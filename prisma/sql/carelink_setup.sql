...
-- -------------------------
-- 2. AUTH TRIGGER
-- Auto-creates a User row whenever someone signs up via Supabase Auth.
-- -------------------------
...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(
      (NEW.raw_app_meta_data->>'role')::public."Role",
      'PCA'::public."Role"
    ),
    'RED',
    NOW(),
...
-- -------------------------
-- 3. JWT ROLE CLAIM HOOK
-- Embeds the user's role into the JWT so middleware can read it
-- without hitting the database on every request.
-- After running this, go to:
--   Supabase Dashboard → Authentication → Hooks
--   Add hook: "Customize Access Token" → function: public.custom_access_token_hook
-- -------------------------
...
DECLARE
  ...
  user_row RECORD;
BEGIN
  SELECT role, "complianceStatus", "facilityId"
  INTO user_row
  FROM public."User"
  ...
  IF user_row IS NOT NULL THEN
    ...
    claims := jsonb_set(claims, '{facilityId}',   to_jsonb(user_row."facilityId"));
    claims := jsonb_set(claims, '{compliance}',   to_jsonb(user_row."complianceStatus"));
  END IF;
...
-- Users
CREATE POLICY "users_read_own"         ON public."User" FOR SELECT USING (auth.uid()::text = id);
CREATE POLICY "users_update_own"       ON public."User" FOR UPDATE USING (auth.uid()::text = id);
CREATE POLICY "admins_read_all_users"  ON public."User" FOR SELECT USING (
  EXISTS (SELECT 1 FROM public."User" u WHERE u.id = auth.uid()::text AND u.role = 'ADMIN')
);
CREATE POLICY "admins_update_any_user" ON public."User" FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public."User" u WHERE u.id = auth.uid()::text AND u.role = 'ADMIN')
);

-- Facilities
CREATE POLICY "admins_read_all_facilities"   ON public."Facility" FOR SELECT USING (
  EXISTS (SELECT 1 FROM public."User" u WHERE u.id = auth.uid()::text AND u.role = 'ADMIN')
);
CREATE POLICY "admins_insert_facility"        ON public."Facility" FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public."User" u WHERE u.id = auth.uid()::text AND u.role = 'ADMIN')
);
CREATE POLICY "facility_admin_update_own"     ON public."Facility" FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public."User" u WHERE u.id = auth.uid()::text AND u."facilityId" = "Facility".id)
);

-- Shifts
CREATE POLICY "workers_read_open_or_own"     ON public."Shift" FOR SELECT USING (
  status = 'PENDING' OR "workerId" = auth.uid()::text
);
CREATE POLICY "facility_read_own_shifts"     ON public."Shift" FOR SELECT USING (
  EXISTS (SELECT 1 FROM public."User" u WHERE u.id = auth.uid()::text AND u."facilityId" = "Shift"."facilityId")
);
CREATE POLICY "admins_read_all_shifts"       ON public."Shift" FOR SELECT USING (
  EXISTS (SELECT 1 FROM public."User" u WHERE u.id = auth.uid()::text AND u.role = 'ADMIN')
);
CREATE POLICY "facility_insert_shifts"       ON public."Shift" FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public."User" u WHERE u.id = auth.uid()::text AND u."facilityId" = "facilityId")
);
CREATE POLICY "workers_accept_shift"         ON public."Shift" FOR UPDATE USING (
  "workerId" IS NULL AND status = 'PENDING'
);
CREATE POLICY "admins_update_any_shift"      ON public."Shift" FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public."User" u WHERE u.id = auth.uid()::text AND u.role = 'ADMIN')
);
...
BEGIN
  ...
  INSERT INTO auth.users (
    ...
    'authenticated', 'authenticated', 'admin@demo.carelink.app', v_pwd,
  ...
  INSERT INTO auth.users (
    ...
    gen_random_uuid(), '00000000-0000-0000-0000-000000000000',
    'authenticated', 'authenticated', 'facility@demo.carelink.app', v_pwd,
    NOW(), '{"provider":"email","providers":["email"]}'::jsonb,
    '{"name":"Demo Facility Manager"}'::jsonb,
    NOW(), NOW(), '', '', '', ''
  )
  ON CONFLICT (email) DO UPDATE SET updated_at = NOW()
  ...
  INSERT INTO auth.users (
    ...
    gen_random_uuid(), '00000000-0000-0000-0000-000000000000',
    'authenticated', 'authenticated', 'nurse@demo.carelink.app', v_pwd,
    NOW(), '{"provider":"email","providers":["email"]}'::jsonb,
    '{"name":"Demo Nurse"}'::jsonb,
    NOW(), NOW(), '', '', '', ''
  )
  ON CONFLICT (email) DO UPDATE SET updated_at = NOW()
